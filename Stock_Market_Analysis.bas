...
last_row = Cells(Rows.Count, 1).End(xlUp).Row
top_row = 2 'summary table index
cnt = 2 'index for intial open price

    For i = 2 To last_row
   If Cells(i, 1).Value <> Cells(i + 1, 1).Value Then
       OpenPrice = Cells(cnt, 3).Value
        cnt = i + 1
      ...
      top_row = top_row + 1 'this ensures that the values found will be properly placed in correct cells
      
    End If
    Next i 'tells the machine to keep looking for all values
